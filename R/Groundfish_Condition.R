...
calc_lw_residuals <- function(len, wt, stratum = NA, bias.correction = TRUE, outlier.rm=FALSE){
  ...
  test <- read.csv(file = "./data/ggplot_ex.csv", stringsAsFactors = FALSE)
  
  len <- test$LENGTH
  wt <- test$WEIGHT
  stratum <- NA#test$STRATUM
  ...
  if(is.na(stratum)[1]) {
    loglength<-log(len)
    logwt<-log(wt)
    lw.mod<-lm(logwt~loglength)
    fitted_wt <- predict(lw.mod, newdata = data.frame(len = len), interval = "c") 
  } else {
    stratum <- factor(stratum)
    loglength <- log(len)
    logwt <-log(wt)
    lw.mod <- lm(logwt~loglength:stratum)
    fitted_wt <- predict(lw.mod, newdata = data.frame(len = len, stratum = stratum), interval = "c") 
  }
  
  # Apply bias correction factor
  if(bias.correction) {
        a <- lw.mod$coefficients[1]
        b <- lw.mod$coefficients[2:length(lw.mod$coefficients)]
        syx <- summary(lw.mod)$sigma
        
        # Bias correction factor
        cf <- exp((syx^2)/2) 
        
        # Apply correction
        fitted_wt <- cf *(exp(pred_wt))
  }
 ...
 # qqPlot(lw.res, main="QQ Plot") #qq plot for studentized resid
  ...
  test1<-car::outlierTest(lw.mod,n.max=Inf,cutoff=Inf,order=FALSE)$bonf.p 
