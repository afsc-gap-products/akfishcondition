...
# Aleutian Islands
ai_biomass <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/ai_biomass.sql", package = "akfishcondition"))) %>%
  dplyr::select(-HAUL_COUNT, -CATCH_COUNT, -INPFC_STRATUM_AREA)

ai_lw <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/ai_length_weight.sql", package = "akfishcondition"))) %>%
...
goa_biomass <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/goa_biomass.sql", package = "akfishcondition"))) %>%
  dplyr::select(-HAUL_COUNT, -CATCH_COUNT, -INPFC_STRATUM_AREA)

# Retrieve GOA length-weight, rename INPFFC_AREA to INPFC_STRATUM and create YEAR field.
goa_lw <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/goa_length_weight.sql", package = "akfishcondition"))) %>%
  dplyr::rename(INPFC_STRATUM = INPFC_AREA) %>%
  dplyr::mutate(YEAR = floor(CRUISE/100),
                ID = paste(CRUISE, VESSEL, HAUL, SPECIMENID, sep = "_")) %>%
...
ebs_biomass <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/ebs_biomass.sql", package = "akfishcondition"))) %>%
  dplyr::mutate(STRATUM = STRATUM*10) # Convert aggregate stratum table names to match EBS strata

# Retrieve EBS length-weight, update STRATUM field and create YEAR field.
ebs_lw <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/ebs_length_weight.sql", package = "akfishcondition"))) %>%
  dplyr::mutate(YEAR = floor(CRUISE/100),
...
nbs_dat <- RODBC::sqlQuery(channel, readr::read_file(system.file("./sql/nbs_length_weight.sql", package = "akfishcondition"))) %>%
  dplyr::mutate(YEAR = floor(CRUISE/100),
                STRATUM = 999,
                ID = paste(CRUISE, VESSEL, HAUL, SPECIMENID, sep = "_")) %>%
  dplyr::select(ID, YEAR, REGION, STRATUM, SPECIMENID, COMMON_NAME, SPECIES_CODE, SEX, LENGTH, WEIGHT)
