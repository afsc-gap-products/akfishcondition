...
Contributed by Sean Rohan^1^ and Bianca Prohaska^1^   
...
```{r figure 2 grid, include=TRUE, echo=FALSE, fig.height=14,fig.width=12,fig.cap="\\label{fig:figs}Figure 2. Weighted length-weight residuals for seven groundfish species and age 1–2 walleye pollock (100–250 mm) collected during AFSC/RACE GAP standard summer bottom trawl surveys of the eastern Bering Sea shelf, 1997-2021. Filled bars denote weighted length-weight residuals using this year's indicator calculation. Error bars denote two standard errors",  message=FALSE, warning=FALSE}
# Set factor levels for plotting order
ebs_ann_mean_resid_df$DISPLAY_NAME <- set_plot_order(ebs_ann_mean_resid_df$COMMON_NAME, REGION = "BS")
ebs_old_indicator_dat$DISPLAY_NAME <- set_plot_order(ebs_old_indicator_dat$DISPLAY_NAME, REGION = "BS")
...
nbs_old_indicator_dat$DISPLAY_NAME <- set_plot_order(nbs_old_indicator_dat$DISPLAY_NAME, REGION = "BS")
...
fig3 <- ggplot() + 
  ...
  #                y = ymeans)) +
  facet_wrap(~DISPLAY_NAME, ncol = 2, scales = "free_y") +
  scale_x_continuous(name = "Year", breaks = c(2010, 2015, 2020), limits = c(2009, 2021)) +
  scale_y_continuous(name = "Length-weight residual (ln(g))") +
  theme_condition_index()
print(fig3)
...
ebs_stratum_resids$DISPLAY_NAME <- set_plot_order(ebs_stratum_resids$COMMON_NAME, REGION = "BS")
...
for(i in 1:length(unique(ebs_stratum_resids$DISPLAY_NAME))) {
  png(paste0("./output/EBS_NBS_condition_", gsub(">", "gt", 
                                                 unique(ebs_stratum_resids$DISPLAY_NAME)[i]), ".png"), 
  ...
  print(
    ggplot(data = nbs_ann_mean_resid_df %>% 
             ...
             dplyr::filter(DISPLAY_NAME == unique(ebs_stratum_resids$DISPLAY_NAME)[i]),
           aes(x = YEAR, 
               y = stratum_resid_mean, 
               fill = STRATUM,
      ...
      geom_hline(yintercept = 0) +
      ggtitle(unique(ebs_stratum_resids$DISPLAY_NAME)[i]) +
      geom_bar(stat = "identity", color = "black", position = "stack") +
      geom_errorbar() +
      facet_wrap(~STRATUM, 
                 ncol = 2, 
