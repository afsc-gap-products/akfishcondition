...
# AI.speciesnames <- c("Walleye pollock","Age 1 Walleye Pollock","Pacific cod","Northern rockfish","Southern rock sole",
#      "Arrowtooth flounder","Pacific Ocean perch","Atka mackerel")
# AI.lwdata <- lwbdata
...
# AI.lwdata <- subset(AI.lwdata, AI.lwdata["INPFC_STRATUM"] != "")
# AI.lwdata$YEAR <- round(AI.lwdata$CRUISE/100, 0)
# 
# # set max YEAR for x-axis formatting
# xmax <- max(lwdata$YEAR) + 1.5
# 
# myplot <- list()
...
# 	tempdata["residuals"] <- calc_lw_residuals(len = tempdata$LENGTH, wt = tempdata$WEIGHT, stratum = tempdata$INPFC_STRATUM, outlier.rm = TRUE, 
#           species_code = tempdata$SPECIES_CODE, year = tempdata$YEAR)
# 	tempdata["residuals.wt"] <- weight_lw_residuals(residuals = tempdata$residuals, year = tempdata$YEAR, stratum = tempdata$INPFC_STRATUM,
#           stratum_biomass = tempdata$AREA_BIOMASS)
...
### this analysis needs to be shifted to the new Rohan functions and use the weighted residuals
...
# 	ymeans = aggregate(tempdata$residuals.wt, by = list(tempdata$YEAR, tempdata$INPFC_STRATUM), mean)
# 	ysd = aggregate(tempdata$residuals.wt, by = list(tempdata$YEAR, tempdata$INPFC_STRATUM), sd)
# 	yn = aggregate(tempdata$residuals.wt, by = list(tempdata$YEAR, tempdata$INPFC_STRATUM), length)
# 	yse = ysd$x/sqrt(yn$x)
# 	data.summary <- data.frame(species = AI.species[i], strata = ymeans$Group.2, yrs = ymeans$Group.1, ymeans = ymeans$x, yn = yn$x, 
#           ysd = ysd$x, yse = yse)
...
# 	
#      p <- ggplot() +
#           geom_bar(data = dat1, aes(x = yrs, y = ymeans, fill = factor(strata, levels = c("Southern Bering Sea","Eastern Aleutians",
#                "Central Aleutians","Western Aleutians"))), stat = "identity", col = "black", width = 1) +
...
#      # i=5 places the legend in the SRS plot (5th species in list) where there was room to display without it overlapping data bars 
#      if(i == 5){
#           p <- p + theme(legend.position = c(.17,.82), legend.title = element_blank(), legend.text = element_text(size = 8),
