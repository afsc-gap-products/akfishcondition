...
ai_ann_mean_resid_df$DISPLAY_NAME <- set_plot_order(ai_ann_mean_resid_df$COMMON_NAME, REGION = "AI")
old_indicator_dat$DISPLAY_NAME <- set_plot_order(old_indicator_dat$DISPLAY_NAME, REGION = "AI")
...
ai_stratum_resids$DISPLAY_NAME <- set_plot_order(ai_stratum_resids$COMMON_NAME, REGION = "AI")

fig3 <- ggplot(data = ai_stratum_resids, 
               aes(x = YEAR, 
                   y = stratum_resid_mean, fill = set_stratum_order(INPFC_STRATUM, REGION = "AI"))) + 
  geom_hline(yintercept = 0) +
...
for(i in 1:length(unique(ai_stratum_resids$DISPLAY_NAME))) {
  
  png(paste0("./output/AI_condition_", gsub(">", "gt", unique(ai_stratum_resids$DISPLAY_NAME)[i]), ".png"), width = 6, height = 7, units = "in", res = 300)
  print(
    ggplot(data = ai_stratum_resids %>% 
             dplyr::filter(DISPLAY_NAME == unique(ai_stratum_resids$DISPLAY_NAME)[i]),
           aes(x = YEAR, 
      ...
      facet_wrap(~set_stratum_order(INPFC_STRATUM, REGION = "AI"), 
                 ncol = 2, 
                 scales = "free_y") +
      ggtitle(unique(ai_stratum_resids$DISPLAY_NAME)[i]) +
      scale_x_continuous(name = "Year") +
      scale_y_continuous(name = "Length-weight residual (ln(g))") +
