...
# Load data
ebs_dat <- readr::read_csv(file = "./data/2020-08-19_ebs_condition_data_2020_ESR.csv")
nbs_dat <- readr::read_csv(file = "./data/2020-08-19_nbs_condition_data_2020_ESR.csv") %>%
...
ebs_ann_mean_resid_df$COMMON_NAME <- set_plot_order(ebs_ann_mean_resid_df$COMMON_NAME)
...
# Set factor levels for plotting order
ebs_stratum_resids$COMMON_NAME <- set_plot_order(ebs_stratum_resids$COMMON_NAME)

fig3 <- ggplot(data = ebs_stratum_resids, 
               aes(x = YEAR, y = weighted_resid_mean, fill = factor(STRATUM))) + 
  geom_hline(yintercept = 0) +
...
nbs_ann_mean_resid_df$COMMON_NAME <- set_plot_order(nbs_ann_mean_resid_df$COMMON_NAME)

fig4 <- ggplot(data = nbs_ann_mean_resid_df, 
...
dev.off()
# write.csv(nbs_ann_mean_resid_df, "EBS_lwdata_by_strata.csv", row.names = FALSE)
```
